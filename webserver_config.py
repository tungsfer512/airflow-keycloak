...
from my_constanst import CLIENT_ID, CLIENT_SECRET, OIDC_ISSUER, PROVIDER_NAME, OIDC_BASE_URL, OIDC_TOKEN_URL, OIDC_AUTH_URL
    ...
    def oauth_user_info(self, provider, response):
        if provider == PROVIDER_NAME:
            ...
            me = jwt.decode(
                token,
                public_key,
                algorithms=["HS256", "RS256", "AES", "RSA-OAEP"],
                options={"verify_aud": False},
            )
            print(me)
            groups = me["resource_access"][CLIENT_ID]["roles"]
            print(groups)
            print("++++++++++++++++++++++++")
            if len(groups) < 1:
